#!/usr/bin/env python3
# =============================================================================
# 模块：因子分析 - 横截面Rank IC计算 + 因子计算函数库  [TBD]
# 文件：TBD/factor_library.py
# 用途：1) 提供因子计算函数库（被 TBD/preprocess_xgb.py、infer_xgb.py、
#          infer_xgb_live.py、backtest_xgb.py 导入），包括 aggregate_to_daily、
#          compute_features、add_alpha101_features、neutralize_cross_section 等。
#       2) 作为可独立运行脚本，计算所有饰品的横截面因子Rank IC（信息系数），
#          评估因子预测能力。输出：IC热力图（PNG）和因子统计摘要（TSV）。
# 使用：python TBD/factor_library.py --data-dir data/daily
