...
> 📌 P1.0 标签重设计 + P1.1 Walk-Forward 验证完成。

### P1.0 标签重设计

改动文件：`TBD/preprocess_xgb.py`

| 改动 | 内容 |
|------|------|
| `--horizon` 参数 | 新增，可选 3d/5d/8d，**默认改为 5d** |
| `compute_target_returns()` | 新增函数：按 horizon 生成 `target_{horizon}d` + `target_up_down_flat_label`（涨/跌/平三分类，阈值 ±1%） |
| `compute_target_rank()` | 改为接收 `target_col` 参数，支持任意 horizon 的排名计算 |
| `required_cols` | 包含三分类标签列 |
| `final_cols` | 输出包含 `target_up_down_flat_label` |

**新增列说明**：

| 列名 | 含义 | 值 |
|------|------|-----|
| `target_5d` | 未来 5 天收益率 | 连续值 |
| `target_rank_pct` | 5 天收益率横截面排名百分位 | 0~1 |
| `target_rank_label` | 排名分桶（NDCG 训练用） | 0/1/3/7/10 |
| `target_up_down_flat_label` | 涨跌平三分类 | 0=跌, 1=平, 2=涨 |

### P1.1 Walk-Forward 验证

脚本：`TBD/p1_walkforward.py`

-  expanding window (train 从 120 天开始，每次测试 20 天，步进 20 天)
-  val 比例 20%，early stopping 200，lr 0.01
- 自包含 `spearman_corr`，无跨模块导入问题
- 数据生成：`preprocess_xgb.py --horizon 5/3`

### P1.1 结果：5d Walk-Forward

| Fold | 测试区间 | best_iter | spearman |
|------|----------|-----------|----------|
| 1 | 2025-07-06 ~ 07-25 | 623 | -0.0155 |
| 2 | 2025-07-26 ~ 08-14 | 9 | +0.0330 |
| 3 | 2025-08-15 ~ 09-03 | 1 | +0.0654 |
| 4 | 2025-09-04 ~ 09-23 | 8 | +0.0261 |
| 5 | 2025-09-24 ~ 10-13 | 23 | +0.0852 |
| 6 | 2025-10-14 ~ 11-02 | 561 | +0.0453 |

**mean spearman = 0.0399 ± 0.0348**（6 folds，全部 199 因子）

### P1.1 结果：3d Walk-Forward

| Fold | 测试区间 | best_iter | spearman |
|------|----------|-----------|----------|
| 1 | 2025-07-06 ~ 07-25 | 20 | -0.0342 |
| 2 | 2025-07-26 ~ 08-14 | 12 | +0.0039 |
| 3 | 2025-08-15 ~ 09-03 | 82 | +0.0464 |
| 4 | 2025-09-04 ~ 09-23 | 816 | +0.0164 |
| 5 | 2025-09-24 ~ 10-13 | 161 | +0.0705 |
| 6 | 2025-10-14 ~ 11-02 | 414 | +0.0094 |

**mean spearman = 0.0187 ± 0.0362**（6 folds，全部 199 因子）

### P1 分析

1. **5d > 3d**：5d 的 mean spearman (0.0399) 高于 3d (0.0187)，说明 5 天的信号更稳定
2. **Walk-Forward < Purged K-Fold**：Walk-Forward 的 0.0399 < P0 Purged K-Fold 的 0.055，但 Walk-Forward 更诚实（ expanding window 无数据泄漏）
3. **显著改善来自 Purged K-Fold 本身**：P0.1 的 Purged K-Fold 已经把数据泄漏排除了，0.055 是真实水平的上界
4. **5d 仍有改善空间**：当前 5d mean 0.0399 还没超过 P0 的 0.055，下一步应在 **21 因子上做 Walk-Forward**（而非 199 因子），预期更接近 0.05 水平
5. **三分类标签已就绪**：`target_up_down_flat_label` 已生成，可随时用于 multiclass XGBoost 实验

### P1 下一步

1. **最高优先**：21 因子上的 Walk-Forward（当前用 199 因子，21 因子预期更好）
2. **三分类实验**：用 `target_up_down_flat_label` 训练 `multi:softmax` 模型，看分类精度 vs 回归 spearman
3. **CS2 独有 Alpha**：P1.1-P1.4（武器箱 EV / Trade Up EV / 花色稀有度 / 事件因子）

### P0+P1 交付物清单

| 文件 | 用途 |
|------|------|
| `TBD/p0_validation.py` | Purged K-Fold + DSR |
| `TBD/p0_validation_report.json` | P0.1 验证结果 |
| `TBD/p0_statistical_test.py` | 统计显著性检验 |
| `TBD/p0_statistical_test_report.json` | P0.5 结果（跨种子 p=0.0013） |
| `TBD/p0_optimized_compare.py` | P0.4 优化双盲对照 |
| `TBD/p0_optimized_compare_report.json` | P0.4 结果 |
| `TBD/p0_ic_analysis.py` | P0.2 IC 分析 |
| `TBD/p0_ic_report.json` | P0.2 IC 排名 |
| **`TBD/preprocess_xgb.py`** | **P1.0 标签重设计（--horizon 3/5/8）** |
| **`TBD/p1_walkforward.py`** | **P1.1 Walk-Forward 验证脚本** |
| `TBD/factor_dataset.parquet` | P0 数据集（8d horizon） |
| `TBD/factor_dataset_p1_5d.parquet` | 5d 目标数据集 |
| `TBD/factor_dataset_p1_3d.parquet` | 3d 目标数据集 |
| `TBD/p1_walkforward_5d_report.json` | 5d WF 结果 |
| `TBD/p1_walkforward_3d_report.json` | 3d WF 结果 |
...
## P0.5 审查与统计显著性检验（2026-06-26）

> 📌 本节是 P0 阶段的**最终审查**：对 P0.1-P0.4 的实现正确性、结论可信度做系统审查，并修复发现的 3 个严重问题。

### 审查发现的 3 个严重问题

| # | 问题 | 影响 | 修复方式 |
|---|------|------|---------|
| 1 | `verdict_cv` 与 `verdict_dsr` 矛盾 | DSR 显示"真实有效"但 CV 显示"假象"，并列容易误读 | 修改 `verdict_dsr` 加 caveat，明确"DSR 通过 ≠ 真实预测能力" |
| 2 | DSR 假设 9 组试验独立 | 9 组 grid search 来自同一份 train/val 切分，sigma_sr 被低估，DSR 被高估 | 新增 `dsr_caveat` 字段说明局限性 |
| 3 | P0.3 双盲对照缺统计显著性检验 | 5 折差异 +0.0209 未做 paired t-test，单次 random_state=42 实验 | 新建 `p0_statistical_test.py` 做配对检验 + 多随机种子实验 |
...
| 模块 | 关键检查项 | 结论 |
|------|-----------|------|
| P0.1 Purged K-Fold | 重叠判断公式 `t' + H >= test_label_start AND t' <= embargo_end` | ✅ 正确 |
| P0.1 embargo | test 后留 2 天禁运 | ✅ 正确 |
| P0.1 DMatrix group | 按 date 分组，sort_values 保证连续 | ✅ 正确 |
| P0.1 val 切分 | 时序切分 + purge 保证无泄漏 | ✅ 正确（P0.4 复核） |
| P0.2 IC 计算 | 截面 Spearman + t-test | ✅ 正确 |
| P0.2 分位收益 | qcut 5 档 + Top-Bottom | ✅ 正确 |
| P0.2 去冗余 | 贪心算法 + corr<0.7 | ✅ 正确 |
| P0.3 双盲对照 | 同函数同参数同 splits | ✅ 正确 |
| P0.3 feature importance | `importance_type="gain"` | ✅ 正确 |
| P0.4 leak check | 修复后窗口重叠判断正确 | ✅ 正确 |

**CS2 数据特性确认**：饰品市场 7x24（无周末），所以 `pd.Timedelta(days=8)` 加 8 自然日 = 8 交易日，purge 范围正确。

### 修复 1-2: verdict 矛盾 + DSR 局限性（p0_validation.py）

修改 [p0_validation.py](file:///f:/cursor/cs2-rank-return-prediction-main/TBD/p0_validation.py) 的 `main()` 综合结论逻辑：

**修改前**（矛盾）：
- `verdict_cv`: "❌ 0.059 大概率是假象"
- `verdict_dsr`: "✅ 0.059 显著高于运气线，真实有效"
- 两者并列容易误读

**修改后**（CV 优先）：
- `verdict_dsr`: "⚠️ 0.059 在 9 组 grid 中显著高于运气线 (DSR 通过), 但 9 组含数据泄漏且非独立试验, DSR 通过 ≠ 真实预测能力. 以 Purged K-Fold 结论为准."
- 新增 `dsr_caveat` 字段：详细说明 DSR 两个局限性
- 新增 `final_verdict` 字段：当 bias > 0.04 时直接判假象，不受 DSR 影响
- `final_verdict` = "❌ 0.059 大概率是数据泄漏假象 (Purged K-Fold bias > 0.04). DSR 通过仅说明含泄漏的 grid 中 0.059 高于运气线, 不代表真实预测能力."

### 修复 3: 统计显著性检验（p0_statistical_test.py）

新建 [p0_statistical_test.py](file:///f:/cursor/cs2-rank-return-prediction-main/TBD/p0_statistical_test.py)，包含两部分：

#### Part 1: 配对检验（基于 P0.3 已有 5 折结果）

| Fold | 199 因子 | 21 因子 | 差异 |
|------|---------|---------|------|
| 1 | -0.0451 | 0.0177 | +0.0627 |
| 2 | 0.0114 | -0.0037 | -0.0152 |
| 3 | -0.0210 | 0.0147 | +0.0357 |
| 4 | 0.1252 | 0.1114 | -0.0138 |
| 5 | -0.0184 | 0.0166 | +0.0349 |

| 统计量 | 值 | 说明 |
|--------|-----|------|
| mean_diff | +0.0209 | 21 因子平均优势 |
| std_diff | 0.0342 | 5 折差异标准差 |
| **paired t-test** | t=1.366, **p=0.2437** | **不显著**（5 折样本太小） |
| Wilcoxon signed-rank | W=3.0, p=0.3125 | 非参数也不显著 |
| Cohen's d | 0.6109 | 中等效应量 |
| 21 因子胜率 | 60% (3/5) | 低于 80% 阈值 |

**Part 1 判定**: ⚠️ 21 因子略优，但 5 折样本太小无法达到统计显著

#### Part 2: 多随机种子实验（5 个种子，每个种子跑 5 折 Purged K-Fold）

配置：val=20%, es=200, lr=0.01（P0.4 最优配置）

| Seed | 199 因子 | 21 因子 | 差异 | 21 胜? |
|------|---------|---------|------|--------|
| 42 | 0.0175 | 0.0534 | +0.0360 | ✓ |
| 0 | 0.0237 | 0.0542 | +0.0305 | ✓ |
| 7 | 0.0219 | 0.0534 | +0.0315 | ✓ |
| 13 | 0.0270 | 0.0513 | +0.0242 | ✓ |
| 2024 | 0.0106 | 0.0606 | +0.0500 | ✓ |

| 跨种子统计 | 值 | 说明 |
|-----------|-----|------|
| 21 因子 mean of means | 0.0546 ± 0.0035 | **非常稳定** |
| 199 因子 mean of means | 0.0201 ± 0.0063 | 波动更大 |
| 差异 mean | +0.0344 ± 0.0096 | 21 因子稳定优势 |
| 21 因子胜率 | **100% (5/5)** | 所有种子都赢 |
| **跨种子配对 t-test** | t=7.99, **p=0.0013** | **高度显著** |

**Part 2 判定**: ✅ 21 因子优势在多随机种子下稳定 (p<0.05, 胜率 100%)

### P0.5 综合结论

1. ✅ **P0.1-P0.4 核心算法实现全部正确**（重叠判断、embargo、DMatrix group、IC 计算、去冗余、双盲对照、leak check）
2. ✅ **修复 verdict 矛盾**: DSR 不再宣称"真实有效"，加 caveat 说明 9 组含泄漏且非独立
3. ✅ **21 因子优势统计显著**: 虽然单次 5 折 p=0.24 不显著（样本小），但跨 5 个种子 p=0.0013 高度显著，胜率 100%
4. 📊 **21 因子稳定性极佳**: 5 个种子下 spearman 在 0.051-0.061 之间（std=0.0035），远比 199 因子稳定
5. 📊 **199 因子过拟合证据**: 5 个种子下 spearman 在 0.011-0.027 之间（std=0.0063），且优化后 Fold 5 best_iter=657 但 sp=0.013

### P0 阶段最终状态

| 指标 | 值 | 说明 |
|------|-----|------|
| **真实 spearman（Purged K-Fold）** | **0.0546 ± 0.0035** | 5 个种子平均（21 因子 + 优化配置） |
| 原 train_xgb.py 报告 | 0.059 | 含数据泄漏 |
| 数据泄漏偏差 | 0.0486 | bias = 0.059 - 0.0104（199 因子原始） |
| 21 vs 199 跨种子 p-value | 0.0013 | 高度显著 |
| 21 vs 199 跨种子胜率 | 100% (5/5) | 完全稳健 |

### 产物文件（P0.5 新增/修改）

| 文件 | 类型 | 用途 |
|------|------|------|
| `TBD/p0_validation.py` | 修改 | 修复 verdict 矛盾 + DSR caveat + final_verdict |
| `TBD/p0_validation_report.json` | 重生成 | 新增 `dsr_caveat` 和 `final_verdict` 字段 |
| `TBD/p0_statistical_test.py` | 新建 | paired t-test + Wilcoxon + 多随机种子实验 |
| `TBD/p0_statistical_test_report.json` | 新生成 | Part 1 + Part 2 完整结果 |

### P0 完成度总览

| 阶段 | 状态 | 关键结论 |
|------|------|---------|
| P0.1 Purged K-Fold + DSR | ✅ 完成 | 0.059 是泄漏假象，真实 ≈ 0.01 |
| P0.2 IC 分析与因子筛选 | ✅ 完成 | 199 → 21 因子（去冗余） |
| P0.3 双盲对照 | ✅ 完成 | 21 因子优于 199（+0.021） |
| P0.4 审查与优化 | ✅ 完成 | val 20% + es 200 + lr 0.01，spearman 0.053 |
| **P0.5 统计显著性检验** | ✅ 完成 | **跨 5 种子 p=0.0013，21 因子优势稳健** |

**P0 阶段全部完成**，可进入 P1（标签重设计 + Walk-Forward 验证）。
